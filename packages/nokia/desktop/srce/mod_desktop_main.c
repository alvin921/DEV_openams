...
static error_t	__on_win_open(t_DesktopGlobData *glob, t_HWindow handle)
{
	...
	wgt_enable_attr(handle, WND_ATTR_HAS_TITLE|WND_ATTR_HAS_SK);
	wnd_set_tb_style(handle, TB_STYLE_IDLE);
	...
	if(g_object_valid(gallery)){
		...
		listctrl_set_rows(gallery, 0, 20, 0);
		listctrl_set_cols(gallery, 1, 0);
